...
class ManifestWriterTest {
    ...
    fun testGenerateEscapedCSV() {
        val manifestRecord = ManifestRecord("\"_id\":{\"declarationId\": \"1234567890\"}", 100000000, "dbwithcomma,", "collectionwithdoublequote\"", "EXPORT")
        val actual = manifestWriter.generateEscapedCSV(manifestRecord)
    ...
    @Test
    fun testManifest() {
        val manifestRecord = ManifestRecord("id1", 100000000, "db1", "collection1", "EXPORT")
        manifestWriter.generateManifest(manifestRecord, "000001")
    ...
    fun testManifestLogsException() {
        ...
        Assert.assertTrue(formattedMessages.contains("Exception while writing id: 'id1' of db: 'db1, collection: collection1' to manifest file in S3"))
