...
if [[ $CREATE_PAGINATED_DATA == "yes" ]]; then
    ...
    if create_sample_data; then
        ...
        for file in *.json.gz.enc; do
            aws_s3 cp $file s3://${S3_BUCKET}/${S3_PREFIX}files-data/
        done

        for file in *.json.encryption.json; do
            aws_s3 cp $file s3://${S3_BUCKET}/${S3_PREFIX}files-metadata/
        done
